...
class DBapp:
    ...
    def __del_from_sl_user(self, vk_id):
        self.session.query(self.searchinglist).where(self.searchinglist.user_id == vk_id).delete()
        self.session.commit()
        print(f'БД __del_from_sl_user: поисковая таблица для {vk_id} очищена')
    ...
    def __ids_from_lists(self, user_id):
        ...
        for item in self.get_unfavorite_list(user_id):
            check_list.append(item[0])
        print(f'БД __ids_from_lists: сформирован список {len(check_list)} ид из черного и белого списков для пользователя {user_id}')
        return check_list
    ...
    def add_to_searching_list(self, match_dic, user_id):
        ...
        self.session.commit()
        print(f'БД add_to_searching_list: для пользователя {user_id} обновлен список соотвествий на {len(match_dic)-len(check_list)} позиций')
    ...
    def get_match(self, index):
        query = self.session.query(self.searchinglist).filter(self.searchinglist.id == index)
        for row in query:
            print(f'БД get_match: найдено соотвествие {row.match_id, row.first_name, row.last_name, row.age, row.sex, row.city}')
            return [row.match_id, row.first_name, row.last_name, row.age, row.sex, row.city]

    def get_first_search(self, user_id):
        index = self.session.query(self.user.first).filter(self.user.vk_id == user_id).all()
        print(f'БД get_first_search: выявлен индекс первого {index}')
    ...
    def get_next_search(self, user_id):
        ...
        self.session.commit()
        index = self.session.query(self.user.actual).filter(self.user.vk_id == user_id).all()[0][0]
    ...
    def get_previous_search(self, user_id):
        first = self.session.query(self.user.first).filter(self.user.vk_id == user_id).one()[0]
        actual = self.get_actual_index(user_id)
        if first >= actual:
            return 0
        self.session.query(self.user).filter(self.user.vk_id == user_id).update({'actual': self.user.actual - 1})
    ...
    def get_actual_index(self, user_id):
        actual = self.session.query(self.user.actual).filter(self.user.vk_id == user_id).one()[0]
        print(f'БД get_actual_index: получен актуальный индекс для пользователя {user_id} {actual}')
        return actual

    def get_match_id_by_id(self, id):
